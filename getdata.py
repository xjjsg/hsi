...
class TradingCalendar:
    ...
    async def is_both_trading(self, target_date: date) -> bool:
        ...
        LOGGER.info(f"交易日检测 {target_date}: A股={'开' if a_share else '休'} / 港股={'开' if hk else '休'} -> {status}")
...
class Scheduler:
    ...
    async def wait_until(self, target: datetime):
        ...
        if wait_seconds > 0:
            LOGGER.info(f"等待 {wait_seconds:.0f} 秒 ({wait_seconds/3600:.2f} 小时) 到 {target.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    async def run(self):
        ...
        while self.running:
            ...
            LOGGER.info(f"下一个窗口: {window_name} @ {first_window_start.strftime('%Y-%m-%d %H:%M:%S')}")
