...
# 指数配置
INDEX_CODES = ["r_hkHSI", "r_hkHSTECH"]
...
class TencentSource:
    ...
    def parse_line(self, line: str) -> Optional[DataEvent]:
        ...
        try:
            ...
            # --- 处理港股指数 (r_hk*) ---
            if "r_hk" in var_part:
                # 格式: v_r_hkHSI="100~Name~Code~Price..."
                # f[2]=Code, f[3]=Price, f[30]=Time(YYYY/MM/DD HH:MM:SS)
                if len(f) < 31:
                    return None
                ...
                # 转换 Symbol (Tencent Code -> System Key)
                if code == "HSI":
                    symbol = "HSI"
                elif code == "HSTECH":
                    symbol = "HZ2083"  # 兼容旧系统 Key
                else:
                    symbol = code

                # 解析时间: 2026/01/30 18:31:03
                t_str = f[30]
                if len(t_str) > 10:
                    try:
                        # Try parsing date to ensure valid format?
                        # Or just split spaces
                        server_ts = t_str.split(" ")[1]  # 取 HH:MM:SS
                    except:
                        server_ts = datetime.now().strftime("%H:%M:%S")
                else:
                    server_ts = datetime.now().strftime("%H:%M:%S")
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession() as session:
            # 请求 ETF 和 指数
            codes_list = list(ETF_CONFIG.keys()) + INDEX_CODES
            ...
            while True:
                ...
                try:
                    if self.is_trading_time():
                        ...
                        await asyncio.sleep(1)  # 休眠到交易时间 check
