...
def run_backtest(miner, combine, val_dates, data_map, device):
    ...
    with torch.no_grad():
        for date in val_dates:
            ...
            # 2. 滑动窗口批处理
            L = len(samples)
            LOOKBACK = 100  # Consistent with train
...
def train():
    ...
    if os.path.exists(PRETRAIN_PATH):
        ...
        try:
            ...
            miner.load_state_dict(state, strict=False)
            print("✅ 预训练权重加载成功。")
    ...
    crit_haz = nn.BCEWithLogitsLoss(reduction="none")
    ...
    # 8. NEW: 计算 baseline_stats (用于 RiskMonitor)
    if COMPUTE_BASELINE_STATS:
        print("\n[PostTrain] 正在计算 RiskMonitor 基准统计...")
        ...
        print(f"✅ Baseline stats 已保存至 {BASELINE_PATH}")
        print(
