...
def main():
    ...
    try:
        ...
        DIM_WHITE = len(WhiteBoxFeatureFactory().get_derived_keys())
        print(f"  -> Features: Raw={DIM_RAW}, White={DIM_WHITE}")

        miner = DeepFactorMinerV5(input_dim_raw=DIM_RAW, out_dim=BLACKBOX_DIM).to(
            DEVICE
        )
        combine = ResidualCombine(white_dim=DIM_WHITE, black_dim=BLACKBOX_DIM).to(
            DEVICE
        )
    ...
    with torch.no_grad():
        for i, s in enumerate(samples):
            ...
            if not z_feats:
                z_feats = [0.0] * len(
                    sorted_keys
                )  # Should not happen with get_derived_keys
            ...
            if len(raw_tgt_buf) == LOOKBACK:
                ...
                b_m = (
                    torch.tensor([mask_buf], dtype=torch.float32)
                    .unsqueeze(-1)
                    .to(DEVICE)
                )  # (1, T, 1)
                ...
                h_k = (
                    torch.sigmoid(out["logit_hazard"]).squeeze(0).cpu().numpy()
                )  # (K,)
