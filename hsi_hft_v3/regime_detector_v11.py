...
class TwoTierRegimeDetector_v11:
    ...
    def __init__(
        ...
        self.min_residence_micro = min_residence_micro
        self.min_residence_action = min_residence_action
        ...
        # 健康度监控
        self.health_monitor = FeatureHealthMonitor(window=100)

        # 字段映射器
        self.mapper = CanonicalFeatureMapper()

        # 评分历史（用于平滑）
        self.illiquid_score_buffer = deque(maxlen=5)
        self.highvol_score_buffer = deque(maxlen=5)
    ...
    def _detect_action_regime(self, timestamp_ms: int) -> Tuple[str, float]:
        ...
        # 竞争选择（需要明显优势）
        if trending_score > 0.4 and trending_score > mr_score + 0.15:
            return "trending", trending_score
        elif mr_score > 0.4 and mr_score > trending_score + 0.15:
